...
mutable_transform_publisher::MutableTransformPublisher::MutableTransformPublisher(
  rclcpp::Node::SharedPtr node, const std::string & yaml_path, const double & period,
  const bool & commit)
: node_(node),
  broadcaster_(node),
  set_transform_server_(node->create_service<mutable_transform_publisher_msgs::srv::SetTransform>(
      "set_transform",
      std::bind(&MutableTransformPublisher::setTransformCallback, this, std::placeholders::_1,
...
bool mutable_transform_publisher::MutableTransformPublisher::add(
  ...
  const std::chrono::duration<double> & period)
{
  ...
  std::unique_ptr<Publisher> pub(new Publisher(
      source, target, period, transform.transform,
      broadcaster_, node_));
...
bool mutable_transform_publisher::MutableTransformPublisher::setTransformCallback(
...
mutable_transform_publisher::MutableTransformPublisher::findPublisher(
...
std::string mutable_transform_publisher::MutableTransformPublisher::makeKey(
  const std::string & source,
  const std::string & target) const
{
...
bool mutable_transform_publisher::MutableTransformPublisher::validate(
